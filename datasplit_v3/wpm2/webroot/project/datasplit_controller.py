...
class DatasplitController(object):
    ...
    def set_sheet_data(self, name, options, table_id, to_file=None, seq_number=None, module_type="workflow", params=None, cluster="isanger", testmod=False):
        ...
        if self.data.client == "client01":  # 线上拆分指定任务投递到isanger上
            if cluster != "":
                self._sheet_data['cluster'] = cluster
            # self._sheet_data['cluster'] = "isanger"
            # self._sheet_data['cluster'] = "sanger" #isanger集群出问题，暂时切到sanger集群投递任务
        return self._sheet_data
    ...
    def set_sheet_data_data_release(self, name, options, table_id, to_file=None, seq_number=None, module_type="workflow", params=None, cluster="isanger", testmod=False):
        self._post_data = web.input()
        new_task_id = self.get_new_id(table_id)
        self._sheet_data = {
            'id': new_task_id,
        ...
        if seq_number:
            self._sheet_data["output"] = self._create_output_dir_data_release(seq_number, new_task_id)
        if to_file:
            self._sheet_data["to_file"] = to_file
        if self.instant:
            self._sheet_data['instant'] = True
    ...
    def _create_output_dir_data_release(self, seq_number, new_task_id):
        """
        设置高通量数据拆分上传文件目录
        """
        # data = web.input()
        ...
        config = Config()
        bucket = config.get_project_region_bucket(project_type="datasplit").rstrip("/")
        target_dir = '{}/{}/{}/{}/{}/'.format(bucket, datetime.datetime.now().strftime("%Y"), seq_number, datetime.datetime.now().strftime("%Y"), new_task_id)
        return target_dir