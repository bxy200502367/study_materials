...
with open(args.inList, 'r') as list:
    ...
    for line in list:
        ...
        if projectType in ['细菌基因组完成图', '真菌基因组精细图', "微生物基因组纯测序", "三代重测序"]:
            trimDataPath = '-'
            if samplePath == '':
                ...
                if samplePath1.is_dir():
                    for subFile in os.listdir(samplePath):
                        ...
                        if subFile.endswith('.fastq'):
                            dataPath = os.path.join(samplePath, subFile)
                        if subFile == "pass.fq.gz":
                            old_file_path = os.path.join(samplePath,subFile)
                            joint_name = samplePath.split("/")[-1]
                            new_file_name =  joint_name + "_pass.fq.gz"
                            new_file_path = os.path.join(samplePath,new_file_name)
                            if os.path.exists(new_file_path):
                                dataPath = new_file_path
                            else:
