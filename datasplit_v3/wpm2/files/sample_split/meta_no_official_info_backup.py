...
class MetaNoOfficialInfoFile(File):
    """
    空检查,用于指定infile类型的文件夹
    """

    def check(self):
        if super(MetaNoOfficialInfoFile, self).check():
            self.is_file()
            self.get_info()
            return True
        return False
    ...
    def get_info(self):
        ...
        if len(set(lane_name_list)) == 1:
            ...
            raise FileError("meta_no_official_info文件中有多个lane")
        if len(set(library_number_list)) == 1:
            self.set_property("library_number", library_number_list[0])
        else:
            raise FileError("meta_no_official_info文件中有多个文库")
        if len(set(library_work_path_list)) == 1:
            ...
            if os.path.exists(fastq_r1_path) and os.path.exists(fastq_r2_path):
                ...
                if len(set(library_s3_path_list)) == 1:
                    ...
                    raise FileError("meta_no_official_info文件中有多个s3路径")
        else:
            raise FileError("meta_no_official_info文件中有多个工作路径")
