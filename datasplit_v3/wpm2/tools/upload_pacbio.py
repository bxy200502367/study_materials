...
class UploadPacbioTool(Tool):
    def __init__(self, config):
        ...
        self._version = 1.0
        self.python3 = "program/Python-3.11.1/bin/"
        self.lib = self.config.SOFTWARE_DIR + "/program/Python-3.11.1/lib/"
        ...
        pbmerge = self.config.SOFTWARE_DIR + "/bioinfo/datasplit/smrtlink/install/smrtlink-release_11.1.0.166339/smrtcmds/bin/pbmerge"
        if os.path.exists(pbmerge):
            self.pbmerge = "bioinfo/datasplit/smrtlink/install/smrtlink-release_11.1.0.166339/smrtcmds/bin/pbmerge"
        else:
            self.pbmerge = "bioinfo/datasplit/smrtlink/install/smrtlink-release_11.1.0.166339/smrtcmds/bin/pbmerge"
        self.perl = "usr/bin/perl"
    ...
    def get_rawdatas_path(self):
        ...
        cmd = "{}/python3 {} -i {} -o {}".format(self.python3, self.get_rawdata,
            ...
            for i in range(1, len(lines)):
                ...
                if raw_path != "" and raw_path != "-":
                    if raw_path in raw_paths:
                        ...
                        self.set_error("样本%s的raw文件:%s重复，请检查" % (item[4], raw_path))
    ...
    def run_trim_fastq(self):
        ...
        for i in self.trim_info:
            ...
            command = self.add_command("trim_fq_seq"+str(i), cmd, shell=True).run()
            self.wait()
            if command.return_code == 0:
            ...
            cmd = "{}/python3 {} -i {} -o {} -n {}".format(self.python3, self.fq_rename,
    ...
    def upload_file(self, old_path, new_name):
        if old_path != "-":
            ...
            new_upload_path = os.path.dirname(new_path) + "/"
            self.transfer.add_upload(old_path, new_upload_path)
        else:
            new_path = "-"
        return new_path
    ...
    def make_md5sum_file(self):
        ...
        with open(md5_path, "wb") as w:
            ...
            # md5dir = os.path.join(self.work_dir,os.path.basename(i).split('.')[0])
                ...
                self.transfer.add_upload(md5_path, os.path.dirname(i)+"/")
            self.transfer.perform()
